...
class BanManager
{
	...
	/**
	 * 
	 * @param string $banFilename Path to the file that stores data about ban management
	 * @param int $faillureTryAuthorized Number of faillure authorized before banish the IP
	 * @param int $banishementTimeout Time of the banishment
	 */
	public function __construct($banFilename, $faillureNbAuthorized=10, $banishementTimeout=1800) {
		...
		$this->banList = json_decode(file_get_contents($this->banFilename));
	}
